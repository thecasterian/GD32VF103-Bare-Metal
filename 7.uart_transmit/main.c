...
int main(void) {
    ...
    GPIOA->CTL1 |= ((0x2UL << GPIO_CTL1_CTL9_Pos) | (0x3UL << GPIO_CTL1_MD9_Pos));
    ...
    /* Set baud-rate to 115200: USARTDIV = PCLK / (16 * 115200) = 4.3402777... where PCLK is PCLK2 for USART0 and is
       8 MHz by default. The integer part of the baud-rate (INTDIV) is 4 and fractional part (FRADIV) is 0.3402777...
       * 16 = 5 (take the nearest 4-bit integer). The error is 0.644%, which is acceptable. */
    USART0->BAUD = ((4UL << USART_BAUD_INTDIV_Pos) | (5UL << USART_BAUD_FRADIV_Pos));
    /* Enable transmitter. */
...
static void send_byte(uint8_t c) {
    ...
    USART0->DATA = (c << USART_DATA_DATA_Pos) & USART_DATA_DATA_Msk;
}
